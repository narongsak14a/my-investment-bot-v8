...
CLOUDFLARE_WORKER_URL = (
    ...
    or "https://dry-voice-2e82.narongsak14.workers.dev/"
)
...
def fetch_portfolio_data():
  ...
  try:
    ...
    portfolio_text += (
        "📌 รายการสินทรัพย์อื่นๆ พร้อมเงื่อนไข/หมายเหตุ (ชีท asset):\n"
    )
    portfolio_text += df_asset.to_string(index=False) + "\n\n"
    portfolio_text += (
        "📌 รายการกองทุน/เงินลงทุน พร้อมเงื่อนไข/หมายเหตุ (ชีท"
        " investment):\n"
    )
...
def fetch_user_needs():
  ...
  try:
    ...
    need_text = (
        "=== [คำสั่งและเป้าหมายวิเคราะห์พิเศษจากผู้ใช้ (ชีท NEED)] ===\n"
    )
...
สร้างตาราง Markdown สรุปรายการสินทรัพย์และกองทุน **โดยต้องดึงทุกรายการที่มีอยู่ในชีท investment มาแสดงในตารางให้ครบทุกบรรทัด ห้ามตกหล่น** (เช่น rmf1, rmf4, kfcashrmf, es-fixedrmf, es-goldrmf-h) และหากมีข้อมูลในชีท asset ให้นำมารวมด้วย โดยให้มีคอลัมน์ดังนี้:
...
**5. คำแนะนำการเติมเงิน/เพิ่มทุน และจัดสัดส่วนใหม่ (DCA & New Money Allocation Plan)**
- **ส่วนที่ควรเพิ่มพอร์ต (Top-up/DCA):** แนะนำว่าหากมีเงินใหม่เข้ามา ควรเน้นเติมเข้ากองทุน/สินทรัพย์ใดเพื่อสร้างการเติบโตหรือกระจายความเสี่ยงให้เหมาะสม
- **ส่วนที่ควรสับเปลี่ยน (Switching Plan):** แนะนำการโยกย้ายเงินจากกองทุนพักเงิน (เช่น kfcashrmf, es-fixedrmf) ไปยังกองทุนเป้าหมายตามสัญญาณเทคนิค EMA12/26
"""    
  return macro_tech_prompt


# ==========================================
# 5. ฟังก์ชันเรียก Gemini API เจนเนื้อหารายงาน
# ==========================================
def generate_ai_report(prompt_text):
  if not GEMINI_API_KEY:
    print(
        "❌ ไม่พบ GEMINI_API_KEY ใน Environment"
        " Variables! สลับไปส่ง Prompt ตรง"
    )
    return prompt_text

  print("🧠 กำลังส่ง Prompt ให้ Gemini AI สร้างบทวิเคราะห์...")
  try:
    model = genai.GenerativeModel("gemini-3.5-flash")
    response = model.generate_content(prompt_text)
    print("✅ Gemini AI สร้างบทวิเคราะห์สำเร็จ!")
...
# ==========================================
# 6. ฟังก์ชันส่งข้อมูลไปยัง Cloudflare Worker
# ==========================================
def send_to_cloudflare(report_content):
  print(f"🤖 กำลังส่งรายงานไปยัง Cloudflare ({CLOUDFLARE_WORKER_URL})...")
  headers = {"Content-Type": "application/json; charset=utf-8"}
  payload = {
      "prompt": report_content,  # ส่งบทวิเคราะห์ภาษาไทยเต็มรูปแบบที่สร้างจาก Gemini
  ...
  try:
    response = requests.post(
        CLOUDFLARE_WORKER_URL, data=json_bytes, headers=headers, timeout=30
    )
    if response.status_code == 200:
      ...
      print(
          f"❌ การส่งข้อมูลล้มเหลว (Status Code:"
...
if __name__ == "__main__":
  # 1. สร้าง Prompt จากข้อมูลชีท
  prompt_result = run_investment_ai_pipeline()

  # 2. ให้ Gemini API ประมวลผลออกมาเป็นเนื้อหารายงาน
  ai_report = generate_ai_report(prompt_result)

  # 3. ยิงบทวิเคราะห์ขึ้น Cloudflare Worker
  send_to_cloudflare(ai_report)
